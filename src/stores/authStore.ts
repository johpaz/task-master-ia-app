...
// Suponiendo que tu interfaz User ya está definida en ../types
...
//   avatar?: string;
//   department?: string;
//   createdAt: Date | string; // Puede ser string si viene de JSON y necesitas parsearlo
...
if (!API_BASE_URL) {
  console.error(
    "FATAL ERROR: VITE_REACT_APP_URL is not defined in your .env file." +
    "Please ensure it's set, e.g., VITE_REACT_APP_URL=http://localhost:3000/api"
  );
  // Podrías lanzar un error o tener un fallback, pero es mejor que esté definida.
}
    ...
    (set, get) => ({
      ...
      login: async (credentials: LoginCredentials) => {
        ...
        try {
          ...
          const data: LoginResponse | { message: string; errors?: any[] } = await response.json();

          if (!response.ok) {
            const errorMessage = (data as { message: string }).message || `Login failed with status: ${response.status}`;
